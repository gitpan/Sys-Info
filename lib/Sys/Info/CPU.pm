...
sub _serve_from_cache {
   my $self    = shift;
   my $context = shift;
   die "Can not happen: cache is not set" if not $self->{CACHE};
   return unless defined $context; # void context
...
Sys::Info::CPU - CPU information.

=head1 SYNOPSIS

   use Sys::Info;
   my $info = Sys::Info->new;
   my $cpu  = $info->cpu(%options);

or

   use Sys::Info::CPU;
   my $cpu = Sys::Info::CPU->new(%options);
...
C<LEVEL> defaults to C<0> and currently only implemented
under C<Linux>.
